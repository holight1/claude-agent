...
**状态**：现行
**日期**：2026-08-21
**触发**：`semantic-code-review` 自述消费者是「读各仓 `CODEX.md` 的独立 reviewer」，而 `CODEX-template.md §禁止事项` 写着「收到『请先读取 CODEX.md』时，只读 `CODEX.md`，不读其他 MD 文件」。照模板初始化的新项目里，该 skill **按设计不可达**。
...
**作用面**：`collab-framework/CODEX-template.md` §禁止事项、`skills/semantic-code-review/SKILL.md` 头部与新增 §4.1。
...
| **可读** | `CODEX.md`、任务文件、`DS.md`、被审查代码与接口两端、项目设计与规格文件、`~/claude-agent/enabled/*/SKILL.md` |
...
不改的代价已经在发生：sim 把 skill 正文抄进项目 `CODEX.md`，两份手工同步的副本从此各自演化。这正是 `documentation-architecture` 列的第一类腐化。
...
- **改 skill，把消费者改成只有架构师终审**，reviewer 侧靠项目 `CODEX.md` 承载内容。放弃：等于承认两份副本手工同步，把腐化制度化。
- **维持禁令，建立「`CODEX.md` 从 skill 生成」的机制**。放弃：生成器要处理项目特有检查与通用检查的合并，复杂度远高于放行一个只读路径；且生成物仍需人工审，收益不抵成本。保留为将来项目数变多后的备选。
...
**未验证**：放行之后 reviewer 是否**真的**会去读 skill（显式或自动加载）。这与 opencode 端到端加载是同一个未闭合的验证面。已知模板改动生效需要新项目初始化或既有项目手工同步 `CODEX.md`——**本仓不改任何项目文件**，sim 侧的 `CODEX.md` 仍是旧措辞。
