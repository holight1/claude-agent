...
- 每个仓各一个 commit，**两条 message 首行都带同一个任务号**——任务号是这两个 commit 唯一的关联键
- **代码仓先提，产出仓后提**；产出仓的 message 里写上代码仓的短 SHA
- 完成区里写**代码仓**的短 SHA
- 两个 commit 是一笔事务：只提了一个就是**部分完成**，不得标已完成

⚠️ **完成区不写产出仓自己的 SHA。** 完成区是产出仓那个 commit 的内容，它的 SHA 要等提交形成之后才存在；写回去又会产生新的 SHA。这是自指，做不到。产出仓那一侧靠**任务号**定位：`git log --grep=<任务号>`。
