...
**重要：不会修改项目的任何文件。**
`CHATGPT.md`、`code-agent/`、知识库、任务文件全部保持原样，随时可用 `/project-add` 重新注册。
...
将从 global memory 中移除 <project-name>：
  ...
  [移除] 项目拓扑块
  [移除] GPT 实例：本地 GPT · <project-name>
  [移除] GPT 实例：远端 GPT · <project-name>（若存在）
  [移除] 仓库与协作表中的对应行

以下内容不受影响：
  [保留] <project-path>/CHATGPT.md
...
- 删除 GPT 实例表中 `本地 GPT · <project-name>` 和 `远端 GPT · <project-name>` 行
- 删除仓库与协作表中对应行

**gpt-registry.md**：
- 删除"当前活跃实例"中对应的 `### N. 本地/远端 GPT · <project-name>` 整块
- 在"历史停用实例"表追加：
  ```
  | `本地 GPT · <project-name>` | <YYYY-MM-DD> | 项目注销 |
  | `远端 GPT · <project-name>` | <YYYY-MM-DD> | 项目注销 |   ← 若存在
  ```
...
- 不删除任何项目文件（CHATGPT.md、code-agent/ 等）
