...
- 若发现轻量问题（测试断言、文档口径、漏改引用、格式/命名、局部边界条件等），直接修复并运行对应验收
- 若发现阻断问题（设计不符、路线不通、较大重构、跨模块接口/ABI 影响等），向用户报告，不继续提交；必要时创建 `*-review-fixes.md`
- 若代码无阻断问题，在后续 memory 更新中注明"代码已 review"

### 3. 判断需要更新哪些文件

对照完成区内容，逐一检查。**任务部分完成时也必须更新 memory 反映当前状态，不要因为"任务未完成"而跳过更新。**
...
若任务包含代码或任务/知识库文件改动，且 review 无阻断问题、轻量问题已修复、验收命令已通过：

- 直接本地 commit，无需再次等待用户确认
