...
实证：同一个病换了四个位置——魔数扫描集（三轮才修成判别式）→ 假设台账（手工枚举 12 项，下一轮才有人问同一句话）→ 审计表自身 → …… 每一次都是「修好上一层之后没有人回头问这一层」。**教训不是「台账漏了一项」，是「修好一个机制之后没有去找同构的其他机制」。**

🔴 **扫描的产出必须是可执行的扫描，不是一张表。** 一张手工枚举的表**不能证明「没有别的手工枚举的表了」**——那是用同一个病去诊断自己。实证：审计交了 8 行表，它自己的 reviewer 在建议区找出了第 9 个；架构师复扫后确认表只是**接近**完备。问题从来不是漏了几行，而是「审计完不完备」这件事本身没有判别式。
